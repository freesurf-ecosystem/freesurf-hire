...
The goal is a Google-style single identity: one account that works across every product. This is already the architecture in place.

One Supabase project acts as the identity provider. It issues a JWT on sign-in. Every product — whether a Cloudflare Worker, a Cloudflare Pages app, or a future backend service — independently validates that JWT using the shared `SUPABASE_JWT_SECRET`. No product needs to call Supabase at request time to verify who the user is.
